...
public class HealthCheck extends BaseTest{
	...
	public void createNewBooking() {
		...
		softAssertUtil
								.assertEquals(response.statusCode(), StatusCode.SUCCESS.code, "Status Code is not 200 instead get "+response.statusCode());
		...
		softAssertUtil
		.assertEquals(getRequestBody().getFirstname(), responseBody.getBooking().getFirstname(), "First Name not as expected");
		
		System.out.println(">>>>> FirstName Validated");
		
		softAssertUtil
		.assertEquals(getRequestBody().getLastname(), responseBody.getBooking().getLastname(), "Last Name not as expected");

		System.out.println(">>>>> LastName Validated");
		
		softAssertUtil
		.assertEquals(getRequestBody().getTotalprice(), responseBody.getBooking().getTotalprice(), "Total Price not as expected");

		System.out.println(">>>>> TotalPrice Validated");
		
		softAssertUtil
		.assertEquals(getRequestBody().isDepositpaid(), responseBody.getBooking().isDepositpaid(), "Deposit Paid not as expected");
		
		System.out.println(">>>>> DepositPaid Validated");
		
		softAssertUtil
		.assertEquals(getRequestBody().getBookingdates().get("checkin"), responseBody.getBooking().getBookingdates().get("checkin"), "CheckIn Date not as expected");

		System.out.println(">>>>> CheckIn Date Validated");
		
		softAssertUtil
		.assertEquals(getRequestBody().getBookingdates().get("checkout"), responseBody.getBooking().getBookingdates().get("checkout"), "CheckOut Date not as expected");
		
		System.out.println(">>>>> CheckOut Date Validated");
		
		softAssertUtil
		.assertEquals(getRequestBody().getAdditionalneeds(), responseBody.getBooking().getAdditionalneeds(), "Additional Needs Date as expected");

		System.out.println(">>>>> AdditionalNeeds Validated");
		
		softAssertUtil.AssertAll();
	}
	
	@Test(description = "RB_10 : Test to validate to update the current Booking with Content-Type : application/json", priority = 8)
	public void updateCurrentBookingPUT() {
		...
		System.out.println("Updated LastName is : "+responseBody.getLastname());
		softAssertUtil.AssertAll();
	}
	
	@Test(description = "RB_13 : Test to validate to update the current Booking partially with Content-Type : application/json", priority = 9)
	public void updateCurrentBookingPATCH() {
		...
		BookingDetailPOJO responseBody = response.as(BookingDetailPOJO.class);
		System.out.println("Updated FirstName is : "+responseBody.getFirstname());
		softAssertUtil.AssertAll();
	}
	
	@Test(description = "RB_16 : Test validate to delete specifig booking", priority=10)
